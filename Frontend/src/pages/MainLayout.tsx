...
const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
  ...
  const handleLogout = () => {
    ...
    if (isConfirm) {
      // 2. Xóa Token và thông tin User khỏi bộ nhớ trình duyệt
      localStorage.removeItem('token');
      localStorage.removeItem('user');
      
      // 3. Cập nhật State để giao diện render lại (mất Avatar, hiện nút Đăng nhập)
      setIsLoggedIn(false);
      
      // 4. Chuyển hướng về trang Đăng nhập (hoặc trang chủ tùy bạn)
      navigate('/');
    }
  };
              ...
              {isLoggedIn ? (
                <div className="flex items-center gap-4 relative">
                  ...
                  <div className="flex items-center group select-none">
                    <div className="w-[35px] h-[35px] bg-[#003399] text-white rounded-full flex items-center justify-center font-bold text-sm mr-2 shadow-sm">
                  ...
                  {/* --- C. ICON LOGOUT (MỚI THÊM) --- */}
                  <div 
                    className="cursor-pointer text-gray-400 hover:text-red-600 transition-colors ml-2"
